...
int[] CreateArrayRndInt(int size, int min, int max)
{
    ...
    Random rnd = new Random();// Random здесь как переменная. переменная для работы со случ числами.
...
int[] GetSomPosNegElem(int[] array)
{
    int sumNeg = 0;
    int sumPos = 0;
...
PrintArray(arr);
int[] sumPosNeg = GetSomPosNegElem(arr);
Console.WriteLine();
